...
module.exports = class Event {
  ...
  */
  validateInput () {
    log.info({ event: this.event }, 'Event.validateInput')

    if (this.secret !== 'I_solemnly_swear_that_I_am_up_to_no_good') {
      throw new Error('secret is wrong: ' + this.event.secret)
    }

    if (this.event_type !== 'metric_alert_monitor') {
      throw new Error('wrong event_type: ' + this.event.event_type)
    }

    if (this.alert_transition !== 'Triggered') {
      throw new Error('wrong alert_transition: ' + this.event.alert_transition)
    }
  }
   ...
   */
  getInstanceId () {
    log.info({ event: this.event }, 'Event.getInstanceId')
